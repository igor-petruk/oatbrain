...
This document defines the mandatory workflow and technical standards for AI agents (Gemini, Claude, etc.) working on the `oatbrain` project. Adherence to these rules ensures architectural integrity and project safety.
...
## 2. Architectural Mandates (SPEC §23)
...
- **No Asyncio**: The project uses the GLib main loop. Do NOT introduce `asyncio` or `gbulb`. Use `ThreadPoolExecutor` and `GLib.idle_add` for background work (SPEC §25).
- **Dependency Control**: Only use Debian-packaged libraries listed in SPEC §2.2. No `pip install` at runtime.

## 3. Testing & Verification (SPEC §30)
...
- **Fakes over Mocks**: Prefer `tests/fakes/` (in-memory implementations) over `unittest.mock` (SPEC §30.3).
