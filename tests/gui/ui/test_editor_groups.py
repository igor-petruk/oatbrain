...
from oatbrain.core.commands.editor import SplitGroupRight, CloseTab, NewTab  # noqa: E402
from oatbrain.core.ports.filestore import VaultPath  # noqa: E402
from oatbrain.core.state import EditorAreaState # noqa: E402
...
def test_complex_split_and_tab_flow(app_and_loop):
    ...
    # Focus group 1 first
    app.editor_area._on_state_change_requested(
        replace(ea, focused_group_index=1)
    )
    _run_loop(loop)
...
def test_tab_reordering_smoke(app_and_loop):
    ...
    # The 'switch-page' signal should trigger _on_tab_switched -> _on_state_change_requested
    _run_loop(loop)
...
def test_mode_switch_targets_correct_tab(app_and_loop):
    """Verify that clicking the mode toggle in an unfocused tab only affects that tab."""
    app, loop = app_and_loop
