...
def test_terminal_paste_keybinding() -> None:
    ...
    print(f"Handled Gdk.KEY_v: {handled}")

    # Try with Gdk.KEY_V (uppercase)
    handled_V = terminal._on_key_pressed(None, Gdk.KEY_V, 0, ctrl_shift)
    print(f"Handled Gdk.KEY_V: {handled_V}")

    assert (
        terminal._paste_from_clipboard.called
    ), "Paste was not triggered by any key combination"
