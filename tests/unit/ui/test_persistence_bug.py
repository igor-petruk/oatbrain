...
def test_editor_mode_recovers_to_preview_when_returning_to_markdown() -> None:
    ...
    state = replace(state, editor=replace(state.editor, open_file=md_path, read_mode=True))
    ...
    state = replace(state, editor=replace(state.editor, open_file=toml_path, read_mode=True))
    editor._update_ui(StateUpdated(state))
    ...
    # The application state still thinks read_mode is True because we didn't update it
    # in step 2 to match the forced state.
    state = replace(state, editor=replace(state.editor, open_file=md_path, read_mode=True))
    editor._update_ui(StateUpdated(state))
